...
const SideBar = ({selectedUser,setSelectedUser}) => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser? "max-md:hidden":''}`}>
        <div className='pb-5'>
         ...
         <div className='bg-[#282142] rounded-full flex items-center gap2 py-3 px-4 mt-5'>
  ...
  <div onClick={()=> setSelectedUser(user)}
       ...
       ${selectedUser && selectedUser._id === user._id ? 'bg-[#282142]/60 border-l-4 border-purple-500' : ''}`}>
         ...
         {/* Notification bubble */}
          {index > 2 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-yellow-500/50'>{index}</p>}
         </div>
            ))} 
        </div>
    </div>
